...
let errFinishAnimationend = (function() {
  var userAgent = navigator.userAgent;

  if (userAgent.indexOf("Edge") > -1) {
    return (
      function(ele) {
          ele.removeClass("error open show")
      }
    )
  }
...
function eventsRegister() {
  let $restartIcon = $('.restart-icon'),
      ...
      $cardsEle = $('.flipper'),
      // 使用事件委托
      $flipperContainer = $('.flipper-container')
  ...
  $flipperContainer.click(function(e) {
    cardsClickEvent(e);
  })
...
function chooseModeDlg() {
  ...
  $('.mode-choose .information-wrapper').addClass('flipInX');
...
function backToMain() {
  $('.mode-choose .information-wrapper').removeClass('flipInX');
  $('.mode-choose .information-wrapper').addClass('flipOutX');
  $('.mode-choose .information-wrapper')[0].addEventListener('animationend', delBacktoAnimation, false);
}
  ...
  while ( (array.length !== 0) && (!array[0].matched)) {
    ...
    if(cardsArray[num_last].imageNum === cardsArray[num_cur].imageNum){
      ...
      $lastEle.addClass("error").bind('animationend', function() {
        /*setTimeout(() => {
          $lastEle.removeClass("open error show")
        }, 250)*/
        errFinishAnimationend($lastEle);
      });
      $currentEle.addClass("error").bind('animationend', function() {
        /*setTimeout(() => {
          $currentEle.removeClass("open error show")
        }, 250)*/
        errFinishAnimationend($currentEle);
      });
      // edge 适用
      /*$lastEle.addClass("error").bind('animationend', function() {
