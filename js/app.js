...
/*
  * TODO:
          12.24
          finish the Erro cards Animation
          spin cards Animation
          change function isMatch, make it List data structure
          when all cards matched, give user information which includes congraulations!
          when clicked 'restart' button, browser will give user dialog to restart game!
          12.25
          add timer function, when all elements matched, it will show how much time spend!
          add rating function, when steps are large than 14, it will vanish a star; when steps are large than 20, it loses a star again; when large than 28, it will lose all stars;
          modify the animation so that can execute well in Edge website! [---dosen't be solved---]
          NEED TO SOLVE:
          - time show from 1s rather than 0s;
          - after clicked 'restart' button, time will not start from 0s immediatelly [---dosen't be solved---]
          - need optimun the result show Animation [---dosen't be solved---]
          - need optimun the code structure to make it understand clearly [---dosen't be solved---]
          12.26
          modify the NEED TO SOLVE(12.25) NO.1
          add some animation in the win information
          NEED TO DO:
          - add the Start interface
  */
